...
class DentalRecord(BaseModel):
    ...
    oral_hygiene_history = db.Column(db.Text)
    allergies = db.Column(db.JSON)
    medical_alerts = db.Column(db.JSON)
    dental_complaints = db.Column(db.Text)
    dental_diagnosis = db.Column(db.Text)
    ...
    charts = db.relationship("DentalChart", back_populates="dental_record", cascade="all, delete-orphan")
    procedures = db.relationship("DentalProcedure", back_populates="dental_record", cascade="all, delete-orphan")
    images = db.relationship("DentalImage", back_populates="dental_record", cascade="all, delete-orphan")
...
class DentalChart(BaseModel):
    ...
    caries = db.Column(db.String(40))
    missing_teeth = db.Column(db.Boolean, default=False)
    filled_teeth = db.Column(db.Boolean, default=False)
    crown_bridge = db.Column(db.Boolean, default=False)
    implant = db.Column(db.Boolean, default=False)
    root_canal = db.Column(db.Boolean, default=False)
...
class DentalProcedure(BaseModel):
    ...
    cost_placeholder = db.Column(db.Numeric(10, 2))
...
class DentalImage(BaseModel):
    ...
    dental_record = db.relationship("DentalRecord", back_populates="images")
...
class OrthodonticCase(BaseModel):
    ...
    diagnosis = db.Column(db.Text)
    malocclusion_class = db.Column(db.String(40))
    appliance_type = db.Column(db.String(100))
    treatment_plan = db.Column(db.Text)
    progress_notes = db.Column(db.Text)
    follow_up_visits = db.Column(db.JSON)
